...
const OUT_CSV     = path.resolve("data/backtest-maker-v6.csv");
...
for (const f of tickFiles) {
  ...
  // v5-style filter: require cache with samples (known resolution structure).
  // Optional tighter filter: only NO-resolved markets.
  if (!cache || !cache.samples?.length) { filtered++; continue; }
  if (REQUIRE_NO_RESOLVED && cache.tokenResolutionValue !== 1 && cache.tokenResolutionValue !== "1") {
    filtered++; continue;
  }
  ...
  // Use cache's resolution bound for marketEndTs (last sample time)
