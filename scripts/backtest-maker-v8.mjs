...
const OUT_CSV     = path.resolve("data/backtest-maker-v8.csv");

// v8: weather-derived resolution + market type/unit filters + monthly breakdown
...
const FILTER_TYPE = argv.filtertype ?? "exact"; // exact | any
const FILTER_UNIT = argv.filterunit ?? "C";     // C | F | any
...
let gatedLiq = 0, gatedCB = 0, gatedBL = 0, gatedMomentum = 0;
...
console.log(`candidates: ${passedCoarse}  gated[liq:${gatedLiq} mom:${gatedMomentum} CB:${gatedCB} BL:${gatedBL}]  scored:${scored}  attempted:${attempted}  filled:${filled}`);
