...
const OUT_CSV     = path.resolve("data/backtest-maker-v9.csv");

// v9: weather-confirmed directional entry (the 0x900e strategy)
...
console.log(`v9: weather observations loaded for ${WEATHER_OBS.size} (city, date) pairs`);

function thresholdCrossed(market, entryTs, buffer) {
  if (!market || market.threshold == null) return false;
  const obs = WEATHER_OBS.get(`${market.city}__${market.date}`);
  if (!obs) return false;
  let maxTemp = -999;
