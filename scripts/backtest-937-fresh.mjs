...
const CFG = {
  ...
  // Peak UTC hours where 937 trades. 16-17h and 00-01h are dominant.
  PEAK_HOURS:   (argv.hours ?? "0,1,16,17").split(",").map(Number),
  // Cities confirmed profitable from analyze-timing.mjs
  CITY_WHITELIST: (argv.cities ?? "Taipei,Toronto,Paris,Lagos,Denver,Singapore,Amsterdam,Jeddah,Miami,Cape Town,Kuala Lumpur").split(","),
  SIDE_ONLY:    argv.side ?? "NO",
  COOLDOWN_SEC: Number(argv.cooldown ?? "900"), // min time between consecutive bids on same market
  // Time-to-resolution window (seconds) — 937 enters between 1h and 3h before resolution
  TTR_MIN_SEC:  Number(argv.ttrmin ?? String(60*60)),   // default 1h
  TTR_MAX_SEC:  Number(argv.ttrmax ?? String(3*60*60)), // default 3h
};
...
function run() {
  ...
  console.log(`        BANKROLL=$${CFG.BANKROLL} TRADE=$${CFG.TRADE_USDC} peakHours=${CFG.PEAK_HOURS.join(",")}`);
  ...
  // Timeline-ordered simulation: merge all samples across markets by time,
  // so bankroll contention is realistic across concurrent markets.
  // Each event carries a reference to its market AND that market's end-of-data
  // timestamp (used as resolution proxy).
  const events = [];
  for (const m of eligible) {
    const city = cityFromTitle(m.title);
    const marketEndTs = m.resolutionTs || m.samples[m.samples.length - 1].t;
    for (const s of m.samples) {
      events.push({ t: s.t, p: s.p, market: m, city, marketEndTs });
    }
  }
  ...
  for (const ev of events) {
    ...
    if (!positions.has(key)) {
      ...
      const ttr = ev.marketEndTs - ev.t; // time-to-resolution seconds
      const inTtrWindow = ttr >= CFG.TTR_MIN_SEC && ttr <= CFG.TTR_MAX_SEC;
      if (inPeak && inBand && inTtrWindow) {
